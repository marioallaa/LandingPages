...
(function () {
    var userAgent = navigator.userAgent.toLowerCase(), initialDate = new Date(), $document = $(document),
        ...
        windowReady = false, isNoviBuilder = false, loaderTimeoutId, plugins = {
    ...
    $(function () {
        ...
        if (plugins.scroller.length) {
            for (var i = 0; i < plugins.scroller.length; i++) {
                ...
                scrollerItem.jScrollPane({autoReinitialise: true});
            }
        }
        ...
        if (plugins.selectFilter.length) {
            ...
            for (i = 0; i < plugins.selectFilter.length; i++) {
                var select = $(plugins.selectFilter[i]), selectStyle = 'html-' + select.attr('data-style') + '-select';
        ...
        if (!isNoviBuilder) {
            for (var i = 0; i < plugins.videoTriggers.length; i++) {
                var videoTrigger = $(plugins.videoTriggers[i]),
                    ...
                    relatedModal = $(videoTrigger.attr('data-target')), modalVideoObj = relatedModal.find('video')[0];
